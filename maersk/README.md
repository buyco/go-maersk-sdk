...
For using other server than the one defined on index 0 set context value `maersk.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `maersk.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `maersk.ContextOperationServerIndices` and `maersk.ContextOperationServerVariables` context maps.
...
Note, each API key must be added to a map of `map[string]APIKey` where the key is: ConsumerKey and passed in as the auth context for each request.
...
auth := context.WithValue(
		context.Background(),
		maersk.ContextAPIKeys,
		map[string]maersk.APIKey{
			"ConsumerKey": {Key: "API_KEY_STRING"},
		},
	)
...
```go
auth := context.WithValue(context.Background(), maersk.ContextAccessToken, "ACCESSTOKENSTRING")
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(oauth2.NoContext, maersk.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
